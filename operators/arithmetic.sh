...
# Tutorial: Arithmatic operators. 
            #Using expr : Here the expr command is used to evaluates the given expressions and display its corresponding output. The `expr` is similar to 'let,' but it does not save the result to a variable. It directly prints the result.
